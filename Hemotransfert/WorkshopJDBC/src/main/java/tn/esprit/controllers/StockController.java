...
clear();
    ...
    @FXML
    private void switchToCentreTab(ActionEvent event) {
        if (tabPane != null) {
        // Récupérez la référence de l'onglet "centre"
        Tab centreTab = tabPane.getTabs().stream()
                .filter(tab -> tab.getText().equals("centre"))
                .findFirst()
                .orElse(null);

        // Si l'onglet "centre" existe, basculez vers cet onglet
        if (centreTab != null) {
            tabPane.getSelectionModel().select(centreTab);
        }
        } else {
            System.err.println("tabPane is null. Make sure it is properly initialized in your FXML file.");
        }
    }
    @FXML
